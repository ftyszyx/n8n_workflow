...
export async function runCustomScript(
    executeFunctions: IExecuteFunctions, itemIndex: number, browser: any,
    page: any, playwright: any): Promise<INodeExecutionData[]> {
  const scriptCode =
      executeFunctions.getNodeParameter('scriptCode', itemIndex) as string;
  ...
  const context = {
    ...
    helpers: {
      ...
      httpRequestWithAuthentication:
          executeFunctions.helpers.httpRequestWithAuthentication?.bind(
              executeFunctions),
      requestWithAuthenticationPaginated:
          executeFunctions.helpers.requestWithAuthenticationPaginated?.bind(
              executeFunctions),
    },
  ...
  vm.on(
      ...
      executeFunctions.getMode() === 'manual' ?
          ...
          (...args: unknown[]) => console.log(
              `[Workflow "${executeFunctions.getWorkflow().id}"][Node "${
                  executeFunctions.getNode().name}"]`,
              ...args) :
          () => {},
  );
